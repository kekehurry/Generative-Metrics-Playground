...
def test_pro_forma():
    ...
    combinations = [
        ('Resident', 'Office', resident_spaces, office_spaces, backend.input_data.amenity_space),
        ('Resident', 'Amenity', resident_spaces, amenity_spaces, backend.input_data.office_space),
        ('Office', 'Amenity', office_spaces, amenity_spaces, backend.input_data.resident_space)
    ]
    
    for comb in combinations:
        X_name, Y_name, X_spaces, Y_spaces, Z_space_fixed = comb
...
def test_return_range():
    # 设置参数范围
    resident_spaces = np.linspace(10, 100000, 100)  # 示例值，可以调整
    office_spaces = np.linspace(10, 100000, 100)    # 示例值，可以调整
    amenity_spaces = np.linspace(10, 50000, 100)    # 示例值，可以调整
